...
BATCH_SIZE = 8
LR = 3e-4
IMG_SIZE = 300          # EfficientNet-B3의 기본 입력 크기
LABEL_SMOOTHING = 0.1
MODEL_NAME = "efficientnet_b3"  # ★ ConvNeXt와 완전히 다른 구조의 모델
GRAD_ACCUM = 4          # 8x4=32 실효 배치
N_FOLDS = 5
...
# ── Model ──
# EfficientNet: ConvNeXt와 다른 방식으로 이미지를 분석
# ConvNeXt는 "큰 커널"로 넓게 보고, EfficientNet은 "효율적 스케일링"으로 정밀하게 봄
# 서로 다른 관점 → 앙상블하면 서로 보완
...
def train_fold(fold, train_df, val_df, train_dir, dev_dir):
    ...
    model = EfficientNetClassifier(pretrained=True).to(DEVICE)
