"""
Step 3: Pseudo-label 모델로 최종 추론 + Temperature Scaling
"""
import os
import sys
import torch
...
from step2_pseudo_label_train import DualViewClassifier, DATA_ROOT, IMG_SIZE, N_FOLDS
...
def main():
    ...
    # 5 folds x 3 TTA = 15개 logits 평균
    all_logits_list = []
    for fold in range(N_FOLDS):
        ckpt = os.path.join(SAVE_DIR, f"best_model_pseudo_fold{fold}.pth")
    ...
    for t in [0.5, 0.7, 0.8, 1.0]:
        ...
        save_path = os.path.join(SAVE_DIR, f"submission_pseudo_T{t:.1f}.csv")
        submission.to_csv(save_path, index=False)
        print(f"T={t:.1f}: range [{probs[:,0].min():.4f}, {probs[:,0].max():.4f}]")

    print("\nDone! 추천 제출 순서: T=0.7 → T=0.8 → T=1.0")
