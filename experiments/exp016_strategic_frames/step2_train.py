"""
Step 2: 전략적 프레임(1,10,20,30) + 듀얼뷰 + Pseudo-Label + SWA

프레임 1~5 대신 1,10,20,30 사용 (불안정 초기 징후 포함)
에폭 50 + SWA로 더 오래 학습
"""
import os
import torch
...
EPOCHS = 50          # 30→50 에폭 (SWA 구간 포함)
SWA_START = 35       # 35 에폭부터 SWA 시작
BATCH_SIZE = 8
LR = 3e-4
IMG_SIZE = 224
LABEL_SMOOTHING = 0.1
...
EXTRA_FRAMES = [1, 10, 20, 30]  # ★ 전략적 프레임 (불안정 초기 징후 포함)
...
def train_fold(fold, train_df, val_df, train_dir, dev_dir):
    ...
    for epoch in range(EPOCHS):
        ...
        print(f"[Epoch {epoch+1:02d}/{EPOCHS}]{swa_tag} "
    ...
    # SWA BatchNorm 업데이트 + 저장
    if epoch >= SWA_START:
        print("  Updating SWA BatchNorm...")
        torch.optim.swa_utils.update_bn(train_loader, swa_model, device=DEVICE)
        swa_score, swa_acc = evaluate(swa_model, val_loader)
        print(f"  SWA model: LogLoss={swa_score:.4f}, Acc={swa_acc:.4f}")
        torch.save(swa_model.module.state_dict(), os.path.join(SAVE_DIR, f"swa_model_fold{fold}.pth"))
        if swa_score < best_score:
            best_score = swa_score
            torch.save(swa_model.module.state_dict(), os.path.join(SAVE_DIR, f"best_model_fold{fold}.pth"))
            print(f"  -> SWA model is better! Using SWA. (LogLoss: {best_score:.4f})")
...
def expand_with_frames(df, data_dir):
    ...
    for _, row in df.iterrows():
        ...
        # 추가 프레임 (front_frame1~5.png) — train만 영상이 있음
...
def main():
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(all_real, real_labels)):
        ...
        print(f"\nFold {fold+1}: train {len(train_real)} → expanded {len(train_expanded)} + pseudo {len(pseudo_df)} = {len(train_fold_df)}")
